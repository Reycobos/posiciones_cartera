...
def fetch_gate_spot_trades(currency_pair: str = None, limit: int = 1000, 
                          days_back: int = 40) -> List[Dict]:
    """
    Obtiene trades spot de Gate.io usando el endpoint /spot/my_trades
    """
    ...
    try:
        ...
        print(f"📦 Se recibieron {len(trades)} trades spot")
        return trades or []
    except Exception as e:
        print(f"❌ Error obteniendo trades spot: {e}")
        return []
...
def save_gate_spot_positions(db_path: str = "portfolio.db", days_back: int = 40) -> int:
    """
