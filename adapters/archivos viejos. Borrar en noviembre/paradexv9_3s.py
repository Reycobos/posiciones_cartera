...
def next_funding_slot_ms(ts_ms: int, offset_h: int = FUNDING_SLOT_OFFSET_H) -> int:
    """Siguiente slot de 8h a partir de ts_ms (ms)."""
...
def _slots_8h_from(start_ms: int, now_ms: int) -> list[int]:
    slots, t = [], start_ms
    while t <= now_ms:
        slots.append(t)
        t += EIGHT_H_MS
...
def _generate_estimated_rows_from_start(sym: str, unsettled: float, start_ms: int) -> list[dict]:
    now_ms = int(time.time() * 1000)
    slots = _slots_8h_from(start_ms, now_ms)
    if not slots:
        return []
...
def _generate_estimated_rows(sym: str, unsettled: float, last_paid_ts_ms: int) -> list[dict]:
    ...
    now_ms = int(time.time() * 1000)
    slots = _slots_8h_from(last_paid_ts_ms, now_ms)
    if not slots:
        return []
    per = unsettled / len(slots)
    asset = "USD"
