...
def fetch_bybit_closed_pnl(
    ...
    """
    Usa la cache unificada de transaction logs para funding.
    CORREGIDO: Divide en ventanas de 7 días para cumplir con límites de Bybit.
    """
    if days <= 0: days = 7
    end_ms = _now_ms()
    ...
    # Bybit solo permite 7 días por request, así que dividimos en ventanas
    window_days = 7
    ...
    while current_start < end_ms:
        current_end = min(current_start + window_ms - 1, end_ms)  # -1 para evitar solapamiento
        ...
        while True:
            ...
            for it in lst:
                ...
                # funding entre created..updated usando cache unificada
                funding_map = _sum_funding_by_symbol(
                    category=category, 
                    currency=currency,
                    start_ms=created_ms, 
