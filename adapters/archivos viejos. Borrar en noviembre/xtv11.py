...
def fetch_xt_open_positions() -> List[Dict[str, Any]]:
    ...
    Devuelve: entry_price, price, mark_price, liquidation_price, notional, margin,
    ...
    params: Dict[str, Any] = {}  # si quieres filtrar: {"symbol": "eth_usdt"}
    ...
    def _calc_unreal(entry: float, mark: float, size: float, side_txt: str) -> float:
        # fallback si la API no trae floatingPL
    ...
    for p in arr or []:
        ...
        # XT usa "positionSize" en contratos (Cont). Sin contractSize aquí,
        # usamos ese valor como "size base" y notional = size * entry como fallback.
        ...
        rpnl  = to_float(p.get("realizedProfit") or 0.0)              # PnL realizado del endpoint
        margin_isolated = to_float(p.get("isolatedMargin") or 0.0)    # útil si es ISOLATED
        ...
        out.append({
            ...
            "price": float(mark),              # fallback: el endpoint no trae last → usamos mark
            ...
            # placeholders coherentes con el dashboard
            "fee": 0.0,
            "funding_fee": 0.0,
        })
