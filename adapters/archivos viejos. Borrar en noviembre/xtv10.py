...
def _calc_unreal(entry: float, mark: float, size: float, side_txt: str) -> float:
    """Unrealized PnL simple si la API no lo trae."""
...
def fetch_xt_open_positions() -> List[Dict[str, Any]]:
    """
    XT — Open positions (NORMALIZADO)
    Endpoint CORRECTO: GET /future/user/v1/position
    Devuelve claves: entry_price, price, mark_price, liquidation_price, notional, margin,
    unrealized_pnl (o calculado), realized_pnl, side, size, symbol, exchange.
    """
    ...
    params: Dict[str, Any] = {}  # si quieres filtrar por symbol: {"symbol": "eth_usdt"}
    ...
    for p in arr or []:
        ...
        # Tamaño: XT documenta "positionSize" en contratos (Cont). Sin contractSize aquí,
        # usamos el mismo valor como "size base" y notional = size * entry (fallback).
        ...
        rpnl  = to_float(p.get("realizedProfit") or 0.0)          # incluye PnL realizado (parciales, fees/funding según exchange)
        margin_isolated = to_float(p.get("isolatedMargin") or 0.0)

        # Fallback si el endpoint no trae unrealized:
        if abs(upnl) < 1e-12:
        ...
        out.append({
            ...
            "price": float(mark),                 # XT no expone last aquí → usamos mark como fallback de "price"
            "mark_price": float(mark),
            "liquidation_price": float(liq or 0.0),

            # magnitudes monetarias
            "notional": float(abs(size) * entry), # sin contractSize disponible en este endpoint
            "margin": float(margin_isolated),     # guardamos por si es ISOLATED (en CROSS suele ser 0)

            # PnLs
            "unrealized_pnl": float(upnl),
            "realized_pnl": float(rpnl),

            # placeholders coherentes con el resto del dashboard
            "fee": 0.0,
            "funding_fee": 0.0,
        })
...
def debug_dump_xt_open_positions_raw(symbol: Optional[str] = None):
    ...
    Útil para verificar campos (floatingPL, realizedProfit, isolatedMargin, etc.)
    """
    cli = _get_perp()
    path = "/future/user" + "/v1/position"
    params: Dict[str, Any] = {}
    ...
    header["Content-Type"] = "application/x-www-form-urlencoded"
    url = cli.host + path
    code, payload, error = cli._fetch(method="GET", url=url, headers=header, params=params, timeout=cli.timeout)
...
if __name__ == "__main__":
    ...
    # en el parser (ya existente)
    parser.add_argument("--opens-raw", action="store_true", help="Imprime el RAW de /future/user/v1/position")
    ...
    if args.opens_raw:
        debug_dump_xt_open_positions_raw(symbol=args.symbol)
    
    # (el resto de flags ya existentes se mantiene igual)
