...
def _sum_fees_from_trades(symbol: str, start_ms: int, end_ms: int) -> float:
    """
    Suma fees (commission) de /fapi/v1/userTrades para el símbolo en la ventana.
    """
    fees = 0.0
    step = 7 * 24 * 3600 * 1000
    t0 = start_ms
    while t0 < end_ms:
        ...
        for t in trades:
            try:
                # En futures, commission viene negativa o positiva; nos quedamos con el costo
    ...
    try:
        ...
        for position in data:
            try:
                ...
                continue
            positions = enrich_open_positions_with_costs(positions, window_days=7, verbose=False)
