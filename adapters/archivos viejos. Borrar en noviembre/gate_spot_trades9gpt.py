...
def save_gate_spot_positions(db_path: str = DB_PATH_DEFAULT, days_back: int = 40, debug: bool = True) -> Tuple[int, int]:
    ...
    for pair, trades in by_pair.items():
        ...
        def _flush_round():
            ...
            # size real = pico máximo de inventario en la ronda
            data['size'] = peak_inventory_base
        
            row = {
                'exchange': 'gate',
                'symbol': normalize_symbol(f"{base}{quote}"),
                'side': 'spotbuy',       # tu convención para UI
                'ignore_trade': 0,
            ...
            total_qty_in_round = 0.0
            peak_inventory_base = 0.0
            # NUEVO (si no lo pusiste ya en cambios anteriores):
            inventory_base = 0.0
            peak_inventory_base = 0.0
        ...
        for f in trades[idx:]:
            ...
            if f.side == 'buy':
                ...
                # cantidad REAL recibida en base (descuenta fee si la fee es en BASE)
                ...
                # el lote FIFO refleja ONLY lo recibido (evita restos por fees en base)
    ...
    if (rem <= dust and total_qty_in_round >= 500) or (not lot_q and sell_left <= 1e-12):
        _flush_round()
        ...
        if lot_q:
            ...
            if sells_occurred and rem_base <= dust:
                # prácticamente cerrada (solo polvo) → cerrar ronda
                _flush_round()
            else:
