...
def fetch_hyperliquid_open_positions():
    """Obtiene posiciones abiertas de Hyperliquid con todos los campos necesarios"""
    try:
        ...
        # 2) Obtener precios mark actuales (meta)
        meta_payload = {"type": "meta"}
        meta_response = requests.post(HYPERLIQUID_API_URL, json=meta_payload, timeout=30)
        meta_data = meta_response.json() if meta_response.ok else {}

        # Crear mapa de precios mark: {coin: mark_price}
        mark_prices = {}
        for universe_item in meta_data.get("universe", []):
            coin = universe_item.get("name", "")
            mark_px = float(universe_item.get("markPx", 0))
            if coin and mark_px > 0:
                mark_prices[coin] = mark_px

        # 3) Obtener funding fees acumulados (últimos 7 días para posiciones abiertas)
        end_time = utc_now_ms()
        ...
        funding_response = requests.post(HYPERLIQUID_API_URL, json=funding_payload, timeout=30)
        funding_data = funding_response.json() if funding_response.ok else []

        # Agrupar funding por símbolo
        funding_by_symbol = defaultdict(float)
        for ev in funding_data:
            delta = ev.get("delta", {})
        ...
        for asset_pos in asset_positions:
            ...
            if szi != 0:
                ...
                mark_px = mark_prices.get(coin, 0)
                unrealized = float(position.get("unrealizedPnl", 0))
                funding_fee = funding_by_symbol.get(coin, 0)
...
def fetch_hyperliquid_all_balances():
    ...
    Salida normalizada para la UI:
      {
        ...
        balance: spot_value + withdrawable_perps,
    ...
    try:
        ...
        return {
            ...
            "balance": perps_withdrawable + spot_value,
            "unrealized_pnl": perps_unreal,
            "spot": spot_value,
