...
def normalize_symbol(sym: str) -> str:
    ...
    # Primero quitar prefijos
    s = re.sub(r'^PERP_', '', s)
    
    # Luego quitar sufijos (USDT, USDC, etc.) en cualquier orden
    s = re.sub(r'[-_]?USDT$', '', s)
    s = re.sub(r'[-_]?USDC$', '', s)
    s = re.sub(r'[-_]?USD$', '', s)
    s = re.sub(r'[-_]?PERP$', '', s)
    
    # Limpiar guiones/barras sobrantes
    s = re.sub(r'[-_/]+$', '', s)
    
    # Si todavía tiene separadores, tomar la primera parte
    if re.search(r'[-_/]', s):
        s = re.split(r'[-_/]', s)[0]
...
def fetch_bingx_open_positions() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for pos in rows:
            ...
            symbol = normalize_symbol(raw_symbol)  # ← NORMALIZADO
...
def _collect_active_symbols_for_closed(days: int = 30, debug: bool = False, max_symbols: int | None = None) -> list[str]:
    """
    SOLUCIÓN MEJORADA: Prioriza el caché y usa límites más inteligentes
    """
    ...
    if cached_symbols:
        ...
        # Si hay caché, NO aplicamos límites estrictos
        if max_symbols is not None and len(symbols) > max_symbols:
            # Pero si es muy grande, mostramos warning
            if debug:
                print(f"⚠️  Caché tiene {len(symbols)} símbolos, limitando a {max_symbols}")
            symbols = symbols[:max_symbols]
            
        return symbols
    
    # 2) FALLBACK: si no hay caché, usar todos los contratos
    if debug:
        print("ℹ️ Caché vacío, usando todos los contratos")
    
    symbols = fetch_bingx_perp_symbols(debug=debug)
    
    # Para el fallback, aplicamos límite más conservador
    if max_symbols is not None and len(symbols) > max_symbols:
        symbols = symbols[:max_symbols]
        if debug:
            print(f"🔒 Limitado a {max_symbols} símbolos (fallback)")
...
def fetch_closed_positions_bingx(
    ...
    autodetect_max_symbols: int | None = 500,   # ← AUMENTADO a 500
    throttle_ms: int = 200,
) -> list[dict]:
    """
...
def save_bingx_closed_positions(
    ...
    # Si no pasan símbolos, usa el caché con límite generoso
    if not symbols:
        symbols = _collect_active_symbols_for_closed(days=days, debug=debug, max_symbols=200)  # ← AUMENTADO
        if not symbols:
...
def debug_cache_status():
    ...
    if cached:
        print(f"   Símbolos: {cached}")
    else:
        print("   ❌ Caché vacío")
    
    # Verificar si los símbolos problemáticos están en caché
    target_symbols = ["AT-USDT", "ENSO-USDT", "MYX-USDT", "KAITO-USDT", "GIGGLE-USDT"]
    missing = [s for s in target_symbols if s not in cached]
    if missing:
        print(f"   ⚠️  Símbolos faltantes en caché: {missing}")
    else:
        print("   ✅ Todos los símbolos objetivo están en caché")
