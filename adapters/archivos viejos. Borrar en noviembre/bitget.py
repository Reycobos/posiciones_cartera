...
def fetch_bitget_funding_fees(limit: int = 50) -> List[Dict[str, Any]]:
    """
    Obtiene historial de funding de Bitget
    Shape EXACTO para frontend Funding
    """
    # Bitget no tiene endpoint directo para funding payments
    # Usamos el interés history como aproximación
    funding_payments = []
    
    try:
        end_time = int(time.time() * 1000)
        start_time = end_time - (30 * 24 * 60 * 60 * 1000)  # 30 días
        
        data = _bitget_request("GET", "/api/v2/mix/account/interest-history", {
            "productType": "usdt-futures",
        ...
        if data.get("code") == "00000":
            for item in data.get("data", {}).get("interestList", []):
                try:
                    # El interés es un costo (negativo)
                    income = -abs(float(item.get("interest", "0")))
                    
                    funding_payments.append({
                        "exchange": "bitget",
                        "symbol": "GENERAL",  # Bitget no asocia interés por símbolo
                        "income": income,
                        "asset": item.get("coin", "USDT"),
                        "timestamp": int(item.get("cTime", "0")),
                        "funding_rate": float(item.get("hourInterestRate", "0")),
                        "type": "FUNDING_FEE"
                    })
                except Exception as e:
                    print(f"❌ Error procesando funding Bitget: {e}")
                    continue
                    
    except Exception as e:
        print(f"❌ Bitget funding fees error: {e}")
