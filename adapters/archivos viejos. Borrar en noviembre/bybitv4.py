...
def _fetch_tx_logs(category="linear", currency="USDT", start_ms: int=0, end_ms: Optional[int]=None, limit: int=50, max_pages: int=50) -> List[dict]:
    ...
    CORREGIDO: funding positivo = gasto (pago), negativo = rebate (cobro) -> mantenemos el signo original.
    """
    if end_ms is None:
        end_ms = _now_ms()
    events: List[dict] = []
...
def _sum_funding_by_symbol(category="linear", currency="USDT", start_ms: int=0, end_ms: Optional[int]=None, symbols_filter: Optional[set]=None) -> Dict[str, float]:
    """
    Devuelve mapping symbol(BASE) -> funding_sum (positivo = cobro, negativo = pago)
    CORREGIDO: mantenemos el signo original de Bybit sin invertirlo
    """
    ...
    for it in lst:
        ...
        try:
            ...
            # MANTENEMOS el signo original - positivo es pago (negativo para nosotros), negativo es cobro (positivo para nosotros)
...
def fetch_bybit_open_positions(category: str="linear", settle_coin: str="USDT", limit: int=200, max_pages: int=5, enrich_funding: bool=True) -> List[dict]:
    ...
    Forma EXACTA B. 
    CORREGIDO: 
    - funding_fee = suma txlog símbolo (manteniendo signo Bybit: positivo=pago, negativo=cobro)
    - fee = realized_pnl - funding_fee (pero con signos corregidos)
    """
    ...
    if enrich_funding and out and symbols_set:
        ...
        for o in out:
            # funding_map: positivo = pago (gasto), negativo = cobro (ingreso)
            f = float(funding_map.get(o["symbol"], 0.0))
            o["funding_fee"] = f  # positivo=pago, negativo=cobro
            
            # CORREGIDO: fee = realized_pnl - funding_fee
            # Si funding_fee es positivo (pago), debería reducir el realized_pnl para calcular fee
...
def fetch_bybit_closed_pnl(category: str="linear", days: int=7, symbol: Optional[str]=None, limit: int=100, max_pages: int=40, currency: str="USDT") -> List[dict]:
    ...
    CORREGIDO: funding_total mantiene signo Bybit (positivo=pago, negativo=cobro)
    """
    if days <= 0: days = 7
    end_ms = _now_ms()
    ...
    while True:
        ...
        for it in lst:
            ...
            # funding entre created..updated (SETTLEMENT logs) - MANTENIENDO signo Bybit
            funding_map = _sum_funding_by_symbol(category=category, currency=currency,
                                                 start_ms=created_ms, end_ms=updated_ms,
                                                 symbols_filter={sym})
            funding_total = float(funding_map.get(sym, 0.0))  # positivo=pago, negativo=cobro
