...
def fetch_funding_bingx(limit=100, start_time=None, end_time=None) -> List[Dict[str, Any]]:
    """
    Wrapper estándar -> /openApi/swap/v2/user/income (incomeType=FUNDING_FEE)
    Devuelve: exchange, symbol, income, asset, timestamp, funding_rate=None, type
    """
    try:
...
def fetch_funding_bingx_readable(limit=100, start_time=None, end_time=None) -> List[Dict[str, Any]]:
    """
    Igual que fetch_funding_bingx, pero añade 'timestamp_hr' con fecha legible.
    """
    recs = fetch_funding_bingx(limit=limit, start_time=start_time, end_time=end_time)
    for r in recs:
