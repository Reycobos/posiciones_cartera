...
class HyperliquidFIFO:
    ...
    def calculate_fifo_pnl(self, block: Dict) -> Dict[str, Any]:
        """Calcula PnL FIFO para un bloque de trades"""
        if not block["fills"]:
            return {}
        
        # Cola FIFO para lotes
        fifo_queue = deque()
        total_fees = Decimal('0')
        entry_notional = Decimal('0')
        exit_notional = Decimal('0')
        
        # Separar fills de entrada y salida según el side del bloque
        entry_fills = []
        exit_fills = []
        ...
        # Calcular PnL FIFO
        fifo_pnl = Decimal('0')
        remaining_exit_fills = exit_fills.copy()
        
        for exit_fill in exit_fills:
            exit_size = Decimal(str(exit_fill.get("sz", "0")))
            exit_price = Decimal(str(exit_fill.get("px", "0")))
            
            while exit_size > 0 and fifo_queue:
                entry_lot = fifo_queue[0]
                entry_size = entry_lot["size"]
                entry_price = entry_lot["price"]
                
                if entry_size <= exit_size:
                    # Usar todo el lote de entrada
                    if block["side"] == "long":
                        fifo_pnl += (exit_price - entry_price) * entry_size
                    else:  # short
                        fifo_pnl += (entry_price - exit_price) * entry_size
                    ...
                    if block["side"] == "long":
                        fifo_pnl += (exit_price - entry_price) * exit_size
                    else:  # short
                        fifo_pnl += (entry_price - exit_price) * exit_size
                    
                    entry_lot["size"] -= exit_size
                    exit_size = 0
        
        # Calcular promedios ponderados
        entry_size_total = sum(Decimal(str(fill.get("sz", "0"))) for fill in entry_fills)
        exit_size_total = sum(Decimal(str(fill.get("sz", "0"))) for fill in exit_fills)
        
        if entry_size_total > 0:
            entry_avg = sum(Decimal(str(fill.get("px", "0"))) * Decimal(str(fill.get("sz", "0"))) 
                          for fill in entry_fills) / entry_size_total
        else:
            entry_avg = Decimal('0')
            
        if exit_size_total > 0:
            close_avg = sum(Decimal(str(fill.get("px", "0"))) * Decimal(str(fill.get("sz", "0"))) 
                          for fill in exit_fills) / exit_size_total
        else:
            close_avg = Decimal('0')
        
        # Tamaño máximo del bloque
        max_size = max([abs(qty) for qty in block["net_quantity_history"]], default=0)
    ...
    def calculate_funding_for_block(self, block: Dict, funding_data: Dict[str, List[Dict]]) -> float:
        ...
        for funding_event in symbol_funding:
            ...
            if open_time <= funding_time <= close_time:
                ...
                # Asumir que el funding se aplica al tamaño máximo de la posición
                position_size = Decimal(str(block.get("size", "0")))
                entry_price = Decimal(str(block.get("entry_price", "0")))
                
                # Calcular funding payment
                notional = position_size * entry_price
                funding_payment = funding_rate * notional
    ...
    def reconstruct_closed_positions(self, days: int = 60) -> List[Dict]:
        ...
        for block in blocks:
            ...
            closed_position = {
                ...
                "pnl": fifo_result["pnl"],  # PnL FIFO de precio
                "realized_pnl": realized_pnl,
                "funding_total": funding_total,
                "fee_total": fifo_result["fee_total"],
                "notional": notional,
                "initial_margin": None,  # Hyperliquid no proporciona esto fácilmente
                "leverage": None,
