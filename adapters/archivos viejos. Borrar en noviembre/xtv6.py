...
if __name__ == "__main__":
    ...
    ap.add_argument("--ws-trades", action="store_true", help="Escucha trades por WebSocket (user_trade)")
    ap.add_argument("--ws-seconds", type=int, default=20, help="Segundos a escuchar el WS")
    ap.add_argument("--ws-symbol", type=str, default=None, help="Filtrar por símbolo (formato exchange, ej. btc_usdt)")
    ap.add_argument("--ws-listen-key", type=str, default="", help="Listen key si tu cuenta lo requiere (sino vacío)")
    ap.add_argument("--ws-out", type=str, default=None, help="Ruta JSONL para guardar crudo del WS")
    ...
    try:
        ...
        if args.ws_trades:
            debug_ws_xt_trades(seconds=args.ws_seconds, symbol=args.ws_symbol,
                       listen_key=args.ws_listen_key, out=args.ws_out)
