...
# - Otros pares: Detecta patrón BUY... -> SELL... (sin vuelta a BUY) y colapsa en 1 posición.
#                Si no aplica, usa FIFO clásico por rondas (inventario=0).
...
def _trade_ts(tr: Dict[str, Any]) -> int:
    ...
    # CSV fallback: 'time' o 'Time' como texto
...
def _fee_usdt(tr: Dict[str, Any]) -> float:
    ...
    Convierte la fee a USDT:
      - fee_currency == 'USDT' => directo
      - fee_currency == base   => fee * price
      - 'Fee'/'fee' en formato '0.0123 ALPACA' o '0.1 USDT' desde CSV
    """
    # API style
    price = float(tr.get("price") or tr.get("Deal price") or 0)
    fee = tr.get("fee")
...
def _amount_from_trade(t: Dict[str, Any]) -> float:
    ...
    if isinstance(amt, (int, float)):
        return float(amt)
    # formato '123.4 ALPACA'
    s = str(amt or "").strip().split()[0:1]
    try:
...
def _single_or_fifo_positions(trades: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Si el flujo es BUY...BUY -> SELL...SELL (sin volver a BUY tras el primer SELL),
    colapsa TODO en una ÚNICA posición. Si no, usa FIFO clásico por rondas.
    """
    if not trades:
        return []
...
def _insert_closed_positions(records: List[Dict[str, Any]], db_path: str = "portfolio.db") -> Tuple[int, int]:
    ...
    # Asegura que times sean enteros (evita problemas de tipos al comparar)
    for r in records:
    ...
    # Upsert manual: UPDATE-then-INSERT
    for r in records:
        # 1) UPDATE por clave natural
        cur.execute("""
...
def process_and_save_gate_spot_positions(all_trades_by_pair: Dict[str, List[Dict[str, Any]]],
    ...
    for cp, trades in all_trades_by_pair.items():
        ...
        if _is_stable_swap_pair(cp):
            ...
            # AQUÍ usamos la función consistente que incluye fast-path + FIFO
