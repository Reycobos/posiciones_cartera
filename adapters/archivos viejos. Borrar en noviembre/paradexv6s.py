...
def normalize_symbol(sym: str) -> str:
    ...
    sym = sym.replace("-USD-PERP", "").replace("_USD_PERP", "")
    sym = sym.replace("-PERP", "").replace("_PERP", "")
    sym = sym.replace("-USD", "").replace("_USD", "")
...
def fetch_paradex_open_positions() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for r in opens:
            ...
            funding_current = _num(r.get("unrealized_funding_pnl"), 0.0)
            price_unreal = unrealized_total - funding_current
            ...
            out.append({
                ...
                "fee": fee_accum,
                "funding_fee": funding_current,
                "realized_pnl": fee_accum + funding_current
            })

            if PRINT_OPEN_POSITIONS:
                p_open_block(EXCHANGE, sym, size, entry, mark, price_unreal,
                           realized_funding=funding_current, total_unsettled=None,
                           notional=notional, extra_verification=False)
        return out
    except Exception as e:
...
def check_existing_positions():
    """Verifica las posiciones ya guardadas en la base de datos"""
    try:
        conn = sqlite3.connect("portfolio.db")
        cursor = conn.cursor()
        
        # Contar posiciones de Paradex
        cursor.execute("SELECT COUNT(*) FROM closed_positions WHERE exchange = ?", (EXCHANGE,))
        count = cursor.fetchone()[0]
        
        # Obtener fechas range
        cursor.execute("SELECT MIN(close_time), MAX(close_time) FROM closed_positions WHERE exchange = ?", (EXCHANGE,))
        min_time, max_time = cursor.fetchone()
        
        print(f"📋 [DEBUG] Posiciones de Paradex en DB: {count}")
        if min_time and max_time:
            print(f"📅 [DEBUG] Rango temporal: {datetime.fromtimestamp(min_time)} to {datetime.fromtimestamp(max_time)}")
        
        conn.close()
        
    except Exception as e:
        print(f"❌ [DEBUG] Error verificando DB: {e}")
        
# =======================
#  Closed positions - Función principal para el adapter
# =======================
