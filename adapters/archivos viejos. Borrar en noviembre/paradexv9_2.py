...
#================    
FUNDING_SLOT_OFFSET_H = 2
def next_funding_slot_ms(ts_ms: int, offset_h: int = FUNDING_SLOT_OFFSET_H) -> int:
    """Devuelve el timestamp (ms) del PRÓXIMO slot de funding de 8h a partir de ts_ms."""
    t = datetime.utcfromtimestamp(ts_ms / 1000.0)
    # ancla del día con la hora del primer funding del día (offset_h)
    anchor = t.replace(hour=offset_h, minute=0, second=0, microsecond=0)
    if t < anchor:
        # si todavía no hemos llegado al primer slot del día, el ancla "previa" es ayer a offset_h
        anchor -= timedelta(days=1)
    # cuántos pasos de 8h han pasado desde el ancla
    passed = int((t - anchor).total_seconds() // (8 * 3600))
    # el siguiente slot es el inmediatamente posterior
    nxt = anchor + timedelta(hours=8 * (passed + 1))
    return int(nxt.replace(tzinfo=timezone.utc).timestamp() * 1000)
...
def _current_cycle_info(market: str, days_back: int = 30) -> dict | None:
    ...
    start_ms = next_funding_slot_ms(last_paid_ms, offset_h=FUNDING_SLOT_OFFSET_H)
