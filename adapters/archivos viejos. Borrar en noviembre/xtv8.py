...
def fetch_xt_transactions(limit: int = 2000,
    ...
    while len(out) < limit:
        ...
        for trade in items:
            ...
            base = normalize_symbol(sym_raw)
            # XT devuelve cantidad como string
            qty = to_float(trade.get("quantity") or trade.get("executedQty") or 0.0)
            price = to_float(trade.get("price") or 0.0)
            fee_raw = trade.get("fee")
            fee = -abs(to_float(fee_raw)) if fee_raw is not None else 0.0  # costo en negativo
            fee_coin = (trade.get("feeCoin") or "USDT").upper()
            tm = (trade.get("takerMaker") or "").upper()  # "TAKER"/"MAKER"
            otype = (trade.get("orderType") or "").upper()
            # Si fee no viene, estimamos por tus reglas
            if fee == 0.0:
            ...
            out.append({
                ...
                "positionSide": pos_side,  # lo necesitamos para el FIFO de bloques
                "price": price,
                "qty": abs(qty),
